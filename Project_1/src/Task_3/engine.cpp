...
Slime* enemyChooseSlime(Slime* playerSlime, vector<Slime> &enemySlimes, vector<bool> choosable) {
    ...
    for (size_t i = 0; i < enemySlimes.size(); i ++) {
        if (choosable[i]) return &enemySlimes[i];
    }
    return nullptr;
}
...
bool compareActions(const shared_ptr<Action>& a, const shared_ptr<Action>& b) {
    ...
    if (a->getType() == CHANGE && b->getType() == CHANGE) {
        return a->getIsPlayer();
    }
    if (a->getType() == SKILL && b->getType() == SKILL) {
...
shared_ptr<Action> enemyChooseAction(vector<Slime> enemySlimes, Slime* enemySlime, Slime* playerSlime, int& revival, int& attack) {
    vector<bool> choosable = checkChoosable(enemySlimes, enemySlime);
    // use potion if possible
    if (revival) {
        for (size_t i = 0; i < enemySlimes.size(); i ++) { // revival potion
            if (!choosable[i] && enemySlime->getName() != enemySlimes[i].getName()) {
                revival -= 1;
                return make_shared<PotionAction>(&enemySlimes[i], REVIVAL);
            }
        }
    }
    if (attack && !playerSlime->isSuppress(enemySlime) && !enemySlime->getDoubleAttack()) { // attack potion
        attack -= 1;
        return make_shared<PotionAction>(enemySlime, ATTACK);
    }
    if (!enemySlime->getDoubleAttack()) { // change slime only if no attack potion
        // use supressing slime if possible
        for (size_t i = 0; i < enemySlimes.size(); i ++) {
            if (choosable[i] && enemySlimes[i].isSuppress(playerSlime)) {
                return make_shared<ChangeAction>(&enemySlimes[i], false);
            }
        }
        Slime* nxtEnemySlime = enemyChooseSlime(playerSlime, enemySlimes, choosable);
        // disuse supressed slime if possible
        if (nxtEnemySlime != nullptr && playerSlime->isSuppress(enemySlime)) {
            return make_shared<ChangeAction>(nxtEnemySlime, false);
        }
    }
    // use skill if not changeable
    int enemySkillId = 0;
    if (enemySlime->isSuppress(playerSlime)) enemySkillId = 1;
    return make_shared<SkillAction>(enemySlime, false, enemySlime->getSkills()[enemySkillId]);
}
...
void play(istream &is, ostream &os) {
    ...
    int enemyRevivalPotion = 1, enemyAttackPotion = 2;
    int roundNumber = 1;
    ...
    while (is) {
        ...
        // enemy choose action
        actions.push_back(enemyChooseAction(enemySlimes, enemySlime, playerSlime, enemyRevivalPotion, enemyAttackPotion));
        ...
        for (size_t i = 0; i < actions.size(); i ++) {
            ...
            if (act.getType() == SKILL) {
                ...
                if (act.getIsPlayer()) playerSlime = slime;
                    ...
                    enemySlime->setDoubleAttack(false); // disable potion
                    enemySlime = slime;
                }
                os << (act.getIsPlayer() ? "You " : "Enemy ") << "sends " << slime->getName() << endl;
            } else if (act.getType() == POTION) {
                Slime* slime = act.getSlime();
                PotionType potionType = act.getPotionType();
                if (potionType == REVIVAL) slime->revive();
                else slime->setDoubleAttack(true);
                os << "Enemy uses " << (act.getPotionType() == ATTACK ? "Attack" : "Revival") << " Potion on " << slime->getName() << endl;
            }
        }
