...
void play(istream &is, ostream &os) {
    ...
    while (is) {
        ...
        // change supressing slime if possible
        for (size_t i = 0; i < enemySlimes.size(); i ++) {
            if (choosable[i] && enemySlimes[i].isSuppress(playerSlime)) {
                actions.push_back(make_shared<ChangeAction>(&enemySlimes[i], false));
                changeFlag = true;
            }
        }
        Slime* nxtEnemySlime = enemyChooseSlime(playerSlime, enemySlimes, choosable);
        // change supressed slime if possible
        if (!changeFlag && nxtEnemySlime != nullptr && playerSlime->isSuppress(enemySlime)) {
            actions.push_back(make_shared<ChangeAction>(nxtEnemySlime, false));
        } else if (!changeFlag) { // use skill if not changeable
            int enemySkillId = 0;
